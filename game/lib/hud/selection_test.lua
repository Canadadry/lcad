...
test("draw() outlines the drag rectangle between start and current point directly via love.graphics, while dragging", function()
...
test("draw_selected() draws a circle at the screen position of each selected vertex, projected through the given mvp, in the given color", function()
    ...
    hud_selection.draw_selected(m, { 1, 2 }, mvp, 100, 100, 3, colors.Yellow)
...
test("draw_selected() draws nothing for an empty selection", function()
    ...
    hud_selection.draw_selected(m, {}, mat4.identity(), 100, 100, 3, colors.Yellow)
end)
